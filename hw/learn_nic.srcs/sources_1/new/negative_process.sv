...
module negative_process #(
    ...
    reg [3:0]  usr_first_be_r = 4'b0;
    reg [3:0]  usr_last_be_r = 4'b0;
    ...
    generate
        for (genvar i = 0; i < 4; i++) begin
            ...
            assign ram_bar0_enb[i] = ram_ctl_en[i];
        end
        ...
        for (genvar i = 0; i < 4; i++) begin
            ...
            assign ram_bar2_enb[i] = ram_mem_en[i];
        end
    endgenerate
            ...
            if (m_axis_cq_tvalid & m_axis_cq_tready) begin
                if (usr_sop_s) begin
                    for (integer i = 0; i < 4; i++) begin
                        case (desc_mm_req_type_s) 
                        'b0000: begin 
                            ram_bar0_addra[i] <= {desc_mm_address_s};   
                            ram_bar2_addra[i] <= {desc_mm_address_s};   
                        end
                        'b0001: begin 
                            ram_bar0_addra[i] <= {desc_mm_address_s};   
                            ram_bar2_addra[i] <= {desc_mm_address_s};   
                ...
                if (c_mwr_64bit_r) begin
                    ...
                    3'b000: begin   
                        ...
                        if (usr_first_be_used_r) begin
                            ram_bar0_addra[desc_mm_target_function_r[2:0]] <= {desc_mm_address_r} + 16; 
                        end
                        else begin
                            ram_bar0_addra[desc_mm_target_function_r[2:0]] <= {desc_mm_address_r}; 
                        end
                    end
                    3'b010: begin   
                        case ({m_axis_cq_tlast, usr_first_be_used_r})
                        ...
                        if (usr_first_be_used_r) begin
                            ram_bar2_addra[desc_mm_target_function_r[2:0]] <= {desc_mm_address_r} + 16; 
                        end
                        else begin
                            ram_bar2_addra[desc_mm_target_function_r[2:0]] <= {desc_mm_address_r};   
                        end
    ...
    reg     [7:0]   cpl_desc_requester_bus;
    reg     [7:0]   cpl_desc_requester_device_function;
    reg             cpl_desc_poisoned_completion;
    reg     [2:0]   cpl_desc_completion_status;
    reg     [10:0]  cpl_desc_dword_count;
    reg             cpl_desc_locked_read_completion;
    reg     [12:0]  cpl_desc_byte_count;
    reg     [1:0]   cpl_desc_at;
    reg     [6:0]   cpl_desc_address;
    reg             cpl_desc_force_ecrc;
    reg     [2:0]   cpl_desc_attr;
    reg     [2:0]   cpl_desc_tc;
    reg             cpl_desc_completer_id_enable;
    reg     [7:0]   cpl_desc_completer_bus;
    reg     [7:0]   cpl_desc_completer_device_function;
    reg     [7:0]   cpl_desc_tag;
    ...
    always @(posedge clk) begin
        ...
        3: begin
            ...
            if (c_mrd_64bit_r) begin    
                if (cpl_desc_cur_dword_count_r <= 1) begin  
                    cpl_done <= 1;
                    s_axis_cc_tlast <= 1;
                end
                else begin
    ...
    enum logic [0:0] {
        IDLE,
        BUSY
    } watchdog_r[3:0][1:0], watchdog_s[3:0][1:0];

    wire watchdog_r_0 = watchdog_r[0][0];
    wire dma_watchdog_0 = dma_watchdog[0][0];
    ...
    generate for (genvar i = 0; i < 4; i++) begin
        for (genvar j = 0; j < 2; j++) begin
            ...
            always_comb begin
                ...
                BUSY: begin
                    if (dma_watchdog_ack[i][0]) begin
                        watchdog_s[i][0] = IDLE;
                    end else begin
                        watchdog_s[i][0] = BUSY;
                    end
                end
                default: watchdog_s[i][0] = IDLE;
                endcase
            end
            ...
            always_comb begin
                ...
                IDLE: begin
                    if (ram_bar0_wea[i] != 4'b0000) begin
                        if ((ram_bar0_addra[i][11:0] == 12'h214)) begin
                            watchdog_s[i][1] = BUSY;
                        end else begin
                            watchdog_s[i][1] = IDLE;
                        end
                ...
                BUSY: begin
                    if (dma_watchdog_ack[i][1]) begin
                        watchdog_s[i][1] = IDLE;
                    end else begin
                        watchdog_s[i][1] = BUSY;
                    end
                end
                default: watchdog_s[i][1] = IDLE;
                endcase
            end
